...
<div class="container mx-auto p-4">
    <h1 class="text-2xl font-bold mb-4">Novo Gasto</h1>
    <form action="{{ route('gastos.store') }}" method="POST" class="space-y-4">
        @csrf
        <div>
            <label class="block">Descrição</label>
            <input type="text" name="descricao" class="border rounded w-full p-2" value="{{ old('descricao') }}">
            @error('descricao')<span class="text-red-500">{{ $message }}</span>@enderror
        </div>
        <div>
            <label class="block">Valor</label>
            <input type="number" step="0.01" name="valor" class="border rounded w-full p-2" value="{{ old('valor') }}">
            @error('valor')<span class="text-red-500">{{ $message }}</span>@enderror
        </div>
        <div>
            <label class="block">Data</label>
            <input type="date" name="data" class="border rounded w-full p-2" value="{{ old('data') }}">
